    ...
    private void decorateBuilderWithAccountMapping(Stack source, DetailedEnvironmentResponse environment, Credential credential, Builder builder) {
        if (source.getType() == StackType.DATALAKE) {
            ...
            if (accountMapping != null) {
                ...
                Map<String, String> groupMapping = awsAccountMappingService.getGroupMapping(source.getRegion(), credential);
                Map<String, String> userMapping = awsAccountMappingService.getUserMapping(source.getRegion(), credential);
