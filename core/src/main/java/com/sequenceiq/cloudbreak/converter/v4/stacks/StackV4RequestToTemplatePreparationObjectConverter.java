    ...
    private void decorateBuilderWithAccountMapping(StackV4Request source, DetailedEnvironmentResponse environment, Credential credential, Builder builder) {
        if (source.getType() == StackType.DATALAKE) {
            ...
            if (accountMapping != null) {
                builder.withAccountMappingView(new AccountMappingView(accountMapping.getGroupMapping(), accountMapping.getUserMapping()));
            } else if (environment.getIdBrokerMappingSource() == IdBrokerMappingSource.MOCK
                    && source.getCloudPlatform() == CloudPlatform.AWS) {
                Map<String, String> groupMapping = awsAccountMappingService.getGroupMapping(source.getPlacement().getRegion(), credential);
                Map<String, String> userMapping = awsAccountMappingService.getUserMapping(source.getPlacement().getRegion(), credential);
                builder.withAccountMappingView(new AccountMappingView(groupMapping, userMapping));
            }
        }
    }
